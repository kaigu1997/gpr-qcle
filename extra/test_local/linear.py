...
def f(x: npt.NDArray[np.double]) -> npt.NDArray[np.double]:
	return 100.0 * np.ones_like(x)


def main(seed: int = 0) -> None:
	rng: np.random.Generator = np.random.Generator(np.random.MT19937(seed))
	...
	for i in np.arange(1.0, 11.0):
		print(f"\nlength < {i} * min{{dist}} = {i * dist}")
		model: gp.GPR | None = opt.gpytorch_train(x_t, y_t, gpytorch.kernels.RBFKernel(1, lengthscale_constraint=gpytorch.constraints.Interval(0.0, i * dist)), se.loocv, False)
		...
		ax.legend()
		fig.savefig(f"linear_{int(i)}.png")
