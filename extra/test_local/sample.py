...
class distribution:
	...
	# constants
	# WIDTH: npt.NDArray[np.double] = np.array([1.0, 1.0])
	# print("Widths are\n", self.WIDTH, sep=None)
...
class data:
	def __init__(self, dist: distribution, draw_sample_pts: collections.abc.Callable[[npt.NDArray[np.double], npt.NDArray[np.double], int], None]) -> None:
		def random_walk() -> tuple[npt.NDArray[np.double], npt.NDArray[np.double]]:
			...
			central_pts: npt.NDArray[np.double] = np.repeat(dist.CENTER, PTS_RATIO, 0)
			...
			acc_ratio /= NUM_STEP
			print("Acceptance ratio for {} central points is within [{}, {}]".format(central_pts.size // distribution.DIM, np.min(acc_ratio), np.max(acc_ratio)), flush=True)
			# N_PTS = 512
			# EXTRA_RATIO = 25
