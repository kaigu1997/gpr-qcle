...
def loocv(
	...
	return ((gp.square_solver(cov_mat, y) / torch.diagonal(gp.square_solver(cov_mat, torch.eye(x.shape[-2])), 0, -2, -1)) ** 2).sum()
...
def loocv_over_var(
	...
	cov_mat: torch.Tensor = cov(x).to_dense()
	return (gp.square_solver(cov_mat, y) ** 2 / torch.diagonal(gp.square_solver(cov_mat, torch.eye(x.shape[-2])), 0, -2, -1)).sum()


def loocv_se_generator(x_all: torch.Tensor, y_all: torch.Tensor) -> opt.LossFuncType:
	def loocv_se(
		cov: gpytorch.kernels.Kernel,
		x: torch.Tensor,
		y: torch.Tensor,
	) -> torch.Tensor:
		return loocv(cov, x, y) + se_generator(gp.default_predict, x_all, y_all)(cov, x, y)
	return loocv_se
