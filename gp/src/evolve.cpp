...
static std::complex<double> gpr_predict(const double x, const double p, const std::size_t RowIndex, const std::size_t ColIndex)
{
	using namespace std::literals::complex_literals;
	const std::size_t ElementIndex = RowIndex * NUM_PES + ColIndex;
	xt::pyarray<double> r{{x, p}};
	py::module_ gp = py::module_::import("gp");
	if (RowIndex == ColIndex)
	{
		return gp.attr("predict")(r, ElementIndex).cast<double>();
	}
	else if (RowIndex < ColIndex)
	{
		return gp.attr("predict")(r, ElementIndex).cast<double>() + 1.0i * gp.attr("predict")(r, ColIndex * NUM_PES + RowIndex).cast<double>();
	}
	else
	{
		return gp.attr("predict")(r, ColIndex * NUM_PES + RowIndex).cast<double>() + 1.0i * gp.attr("predict")(r, ElementIndex).cast<double>();
	}
}

void evolve(xt::pyarray<double>& pts, xt::pyarray<double>& density, const std::size_t num_pts, const double interval) noexcept
{
	using namespace std::literals::complex_literals;
	static constexpr Direction drc = Direction::Forward;
	const xt::xtensor<std::size_t, 1> ticks = xt::arange(static_cast<std::size_t>(std::round(interval / dt)));
	// map to xadapt
	assert(pts.size() % (PHASEDIM * NUM_ELM) == 0);
	const xt::xtensor<std::size_t, 1> points = xt::arange(pts.size() / (PHASEDIM * NUM_ELM));
	for (const std::size_t iTick: ticks)
	{
		for (const std::size_t iPES: range{0, NUM_PES})
		{
			for (const std::size_t jPES: range{0, NUM_PES})
			{
				const std::size_t ElementIndex = iPES * NUM_PES + jPES, SymmetricElementIndex = jPES * NUM_PES + iPES;
				if (iPES <= jPES)
				{
					std::for_each(
						...
						points.cbegin(),
						points.cend(),
						[&pts, &density, &ticks, iPES, jPES, ElementIndex, SymmetricElementIndex](const std::size_t idx) noexcept -> void
						{
							double& x0 = pts(ElementIndex, idx, 0);
							double& p0 = pts(ElementIndex, idx, 1);
							if (is_coupling(x0, p0))
							{
								// evolve
								const auto [x2, p1] = adiabatic_evolve(x0, p0, dt / 2.0, drc, iPES, jPES);
								std::tie(x0, p0) = adiabatic_evolve(x2, p1, dt / 2.0, drc, iPES, jPES);
								// predict
								const std::complex<double> den_pred = non_adiabatic_evolve_predict(x0, p0, density(ElementIndex, idx), gpr_predict, iPES, jPES);
								density(ElementIndex, idx) = den_pred.real();
								if (iPES != jPES)
								{
									density(SymmetricElementIndex, idx) = den_pred.imag();
								}
							}
							else
							{
								...
								if (iPES != jPES)
								{
									const std::complex<double> den_pred = std::complex<double>(density(ElementIndex, idx), density(SymmetricElementIndex, idx)) * std::exp(-calculate_omega0(x0, x2, drc, iPES, jPES) * dt * 1.0i);
									density(ElementIndex, idx) = den_pred.real();
									density(SymmetricElementIndex, idx) = den_pred.imag();
								}
								std::tie(x0, p0) = std::tie(x2, p1);
							}
						}
					);
				}
				...
				{
					xt::view(pts, ElementIndex, xt::all(), xt::all()) = xt::view(pts, SymmetricElementIndex, xt::all(), xt::all());
				}
			}
		}
		py::module_::import("gp").attr("update")(pts, density, num_pts);
	}
}
