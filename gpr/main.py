...
def main() -> None:
	...
	all_pts: npt.NDArray[np.double] = np.tile(sample.normal_sample(NUM_PTS, r0, sigma_r0)[np.newaxis], (pes.NUM_ELM, 1 + NUM_XTR_RATIO, 1))
	sample.sample_extra_points(NUM_PTS, all_pts)
	sample.sample_central_points(NUM_PTS, all_pts)
	sample.sample_extra_points(NUM_PTS, all_pts)
	...
	with open("points.txt", 'w', encoding="UTF-8") as pts_f,\
		...
		def train_pred_draw(iTick: int) -> None:
			...
			for iPES in range(pes.NUM_PES):
				for jPES in range(iPES + 1):
					...
					if iPES != jPES:
						...
						y_all.imag = 0
			evolving_errors: npt.NDArray[np.double] = np.sum((real_part(y_all) - real_part(all_density)) ** 2, -1)
			# fit
			...
			mca.update_pts(all_pts[:, :NUM_PTS], predictors.predict)
		...
		def draw(iTick: int) -> typing.Iterable[matplotlib.artist.Artist | typing.Iterable[matplotlib.artist.Artist]]:
			...
			# if DoResample:
			# 	sample.sample_central_points(NUM_PTS, all_pts, all_density)
			sample.sample_extra_points(NUM_PTS, all_pts, predictors)
			pred_starting_idx: int = NUM_PTS
			for iPES in range(pes.NUM_PES):
				for jPES in range(iPES + 1):
					ElementIndex: int = iPES * pes.NUM_PES + jPES
			...
			predictors.update(all_pts, all_density, NUM_PTS, get_scale(real_part(all_density)))
			return fig, axs
	...
	for iPlot in range(axs.size):
		...
		for iElement in range(pes.NUM_ELM):
			ax.semilogy(np.arange(total_ticks) * output_interval, error[:, iPlot, iElement], label=gp.get_RI_label(iElement // pes.NUM_PES, iElement % pes.NUM_PES))
		ax.set_xlabel("Time")
		ax.set_ylabel("Error")
		ax.legend()
	...
	for iPlot in range(axs.size):
		...
		for iElement in range(pes.NUM_ELM):
			if iPlot < pes.PHASEDIM:
				ax.plot(np.arange(param_loss_scale_total_ticks) * dt, param_loss_scale[:, iElement, iPlot], label=gp.get_RI_label(iElement // pes.NUM_PES, iElement % pes.NUM_PES))
			else:
				ax.semilogy(np.arange(param_loss_scale_total_ticks) * dt, param_loss_scale[:, iElement, iPlot], label=gp.get_RI_label(iElement // pes.NUM_PES, iElement % pes.NUM_PES))
		ax.set_xlabel("Time")
	...
	for iRow in range(axs.shape[0]):
		for iCol in range(axs.shape[1]):
			...
			if iCol == 0: # population
				...
				for iElement in range(pes.NUM_ELM):
					ax.plot(np.arange(total_ticks) * output_interval, averages[:, iRow, pes.NUM_PES + pes.PHASEDIM + 2 + iElement], label=gp.get_RI_label(iElement // pes.NUM_PES, iElement % pes.NUM_PES))
