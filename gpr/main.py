...
if __name__ == "__main__":
	...
	def train_pred_draw(iTick: int):
		...
		for iPES in range(pes.NUM_PES):
			for jPES in range(0, iPES + 1):
				...
				interpolator_re: scipy.interpolate.RegularGridInterpolator = scipy.interpolate.RegularGridInterpolator((x_grids, p_grids), data[iTick, ElementIndex].real, 'cubic')
				if iPES == jPES:
					y_all.real[ElementIndex] = interpolator_re(all_pts[ElementIndex]) * scale[ElementIndex]
				else:
					y_all.real[ElementIndex] = interpolator_re(all_pts[ElementIndex]) * scale[jPES * pes.NUM_PES + iPES]
					interpolator_im: scipy.interpolate.RegularGridInterpolator = scipy.interpolate.RegularGridInterpolator((x_grids, p_grids), data[iTick, ElementIndex].imag, 'cubic')
					y_all.imag[ElementIndex] = interpolator_im(all_pts[ElementIndex]) * scale[ElementIndex]
					y_all[jPES * pes.NUM_PES + iPES] = np.conj(y_all[ElementIndex])
		print(np.max(np.abs(real_part(y_all)), -1))
		evolving_errors[iTick] = np.sum((real_part(y_all) - real_part(all_density) * scale[:, np.newaxis]) ** 2, -1)
		# fit
		...
		original_errors[iTick] = np.sum((real_part(pred) / np.where(scale == 0.0, 1.0, scale)[:, np.newaxis, np.newaxis] - real_part(data[iTick])) ** 2, (-2, -1))
		...
		for iPlot in range(NUM_PLOTS):
			def contour_data() -> npt.NDArray[np.cdouble]:
				match iPlot:
					case 0 | 1:
						return pred
					case 2:
						return data[iTick] * scale[:, np.newaxis, np.newaxis]
					case 3:
						return pred - data[iTick] * scale[:, np.newaxis, np.newaxis]
					case _:
						raise NotImplemented("Unexpected Plot for Contour")
			ct_data: npt.NDArray[np.double] = real_part(contour_data())
			for iElement in range(pes.NUM_ELM):
				...
				ax.contourf(
					...
					locator=LOCATOR)
				if iPlot == 0:
					ax.scatter(all_pts[iElement, :NUM_PTS, 0], all_pts[iElement, :NUM_PTS, 1], s=0.5, c='black')
		...
		for iPES in range(pes.NUM_PES):
			for jPES in range(pes.NUM_PES):
				if iPES == jPES:
					np.savetxt(den_f, gp.predict(all_pts[iPES * pes.NUM_PES + jPES], iPES * pes.NUM_PES + jPES).real.reshape(1, -1) / (1.0 if scale[iPES * pes.NUM_PES + jPES] == 0.0 else scale[iPES * pes.NUM_PES + jPES]))
				elif iPES < jPES:
					np.savetxt(den_f, gp.predict(all_pts[jPES * pes.NUM_PES + iPES], jPES * pes.NUM_PES + iPES).real.reshape(1, -1) / (1.0 if scale[iPES * pes.NUM_PES + jPES] == 0.0 else scale[iPES * pes.NUM_PES + jPES]))
				else:
					np.savetxt(den_f, gp.predict(all_pts[iPES * pes.NUM_PES + jPES], iPES * pes.NUM_PES + jPES).imag.reshape(1, -1) / (1.0 if scale[iPES * pes.NUM_PES + jPES] == 0.0 else scale[iPES * pes.NUM_PES + jPES]))
	...
	def draw(iTick: int):
		...
		for i in range(int(round(OUTPUT_INTERVAL / DT))):
			evolve.evolve(all_pts, all_density, MASS, DT, gp.predict)
