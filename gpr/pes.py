...
MODEL: typing.Final = Model.DAC
NUM_PES: typing.Final = 2
NUM_ELM : typing.Final= NUM_PES * NUM_PES
NUM_TRIG: typing.Final = NUM_PES * (NUM_PES + 1) // 2
DIM: typing.Final = 1
PHASEDIM: typing.Final = DIM * 2
...
tril_element_indices: typing.Final[npt.NDArray[np.int_]] = tril_row_indices * NUM_PES + tril_col_indices
...
flatten_tril_index: typing.Final[npt.NDArray[np.int_]] = lower_triangular_to_full(np.arange(NUM_TRIG, dtype=np.int_))
...
class InitialDistribution:
	...
	def __init__(
		...
		self.__r0: typing.Final[npt.NDArray[np.double]] = r0
		self.__sigma_r0: typing.Final[npt.NDArray[np.double]] = sigma_r0
		wavefunction_weight_phase: typing.Final[npt.NDArray[np.cdouble]] = (initial_population * np.exp(1.0j * initial_phase_factor)).astype(np.cdouble)
		weight_phase: typing.Final[npt.NDArray[np.cdouble]] = (wavefunction_weight_phase.conj()[:, np.newaxis] * wavefunction_weight_phase / np.sum(initial_population ** 2)) # shape of (NUM_PES, NUM_PES)
		self.__weight_phase: typing.Final[npt.NDArray[np.cdouble]] = (weight_phase + weight_phase.T.conj()) / 2.0 # remove numerical error, shape of (NUM_PES, NUM_PES)
		self.__factors: typing.Final[npt.NDArray[np.cdouble]] = self.__weight_phase.reshape(-1) / (2.0 * math.pi) ** DIM / self.__sigma_r0.prod() # divide by normalization factor, shape of (NUM_ELM,)
...
class potential:
	SAC_A: typing.Final = 0.01
	SAC_B: typing.Final = 1.6
	SAC_C: typing.Final = 0.005
	SAC_D: typing.Final = 1.0
	DAC_A: typing.Final = 0.10
	DAC_B: typing.Final = 0.28
	DAC_C: typing.Final = 0.015
	DAC_D: typing.Final = 0.06
	DAC_E: typing.Final = 0.05
	ECR_A: typing.Final = 6e-4
	ECR_B: typing.Final = 0.10
...
def adiabatic_potential(x: npt.NDArray[np.double]) -> npt.NDArray[np.double]:
	...
	NotImplementedError
		In case the model is unknown
	"""
	V: typing.Final[npt.NDArray[np.double]] = diabatic_potential(x)
	match NUM_PES:
		case 2:
			V00: typing.Final[npt.NDArray[np.double]] = V[..., 0, 0] # ...
			V01: typing.Final[npt.NDArray[np.double]] = V[..., 0, 1] # ...
			V11: typing.Final[npt.NDArray[np.double]] = V[..., 1, 1] # ...
			discriminant: typing.Final[npt.NDArray[np.double]] = np.sqrt(np.square(V00 - V11) + np.square(2.0 * V01)) # ...
			return ((V00 + V11)[..., np.newaxis] + np.array([-1.0, 1.0], dtype=np.double) * discriminant[..., np.newaxis]) / 2.0 # ... * N
		case _:
			raise NotImplementedError("Model NOT Implemented!")
...
def diabatic_to_adiabatic_matrices(x: npt.NDArray[np.double]) -> npt.NDArray[np.double]:
	...
	NotImplementedError
		In case the model is unknown
	"""
	V: typing.Final[npt.NDArray[np.double]] = diabatic_potential(x) # ... * N * N
	match NUM_PES:
		case 2:
			V00: typing.Final[npt.NDArray[np.double]] = V[..., 0, 0] # ...
			V01: typing.Final[npt.NDArray[np.double]] = V[..., 0, 1] # ...
			...
			diag_indices: typing.Final[npt.NDArray[np.int_]] = np.argwhere(V01 == 0)
...
def adiabatic_coupling(x: npt.NDArray[np.double]) -> npt.NDArray[np.double]:
	...
	"""
	E: typing.Final[npt.NDArray[np.double]] = adiabatic_potential(x) # ... * N
	F: typing.Final[npt.NDArray[np.double]] = adiabatic_force(x) # ... * D * N * N
...
def force_basis_force(x: npt.NDArray[np.double], dim: int | None = None) -> npt.NDArray[np.double]:
	...
	force_dia: typing.Final[npt.NDArray[np.double]] = diabatic_force(x) # ... * D * N * N
	match NUM_PES:
		case 2:
			f00: typing.Final[npt.NDArray[np.double]] = tensor_slice(force_dia, dim, 0, 0) # ...( * D)
			f01: typing.Final[npt.NDArray[np.double]] = tensor_slice(force_dia, dim, 0, 1) # ...( * D)
			f11: typing.Final[npt.NDArray[np.double]] = tensor_slice(force_dia, dim, 1, 1) # ...( * D)
			discriminant: typing.Final[npt.NDArray[np.double]] = np.sqrt(np.square(f00 - f11) + np.square(2.0 * f01)) # ...( * D)
			return ((f00 + f11)[..., np.newaxis] + np.array([-1.0, 1.0], dtype=np.double) * discriminant[..., np.newaxis]) / 2.0 # ...( * D) * N
		case _:
			raise NotImplementedError("Model NOT Implemented!")
...
def diabatic_to_force_basis(x: npt.NDArray[np.double], dim: int | None = None) -> npt.NDArray[np.double]:
	...
	match NUM_PES:
		case 2:
			...
			f11: typing.Final[npt.NDArray[np.double]] = tensor_slice(force_dia, dim, 1, 1) # ...( * D)
			discriminant: typing.Final[npt.NDArray[np.double]] = np.sqrt(np.square(f00 - f11) + np.square(2.0 * f01)) # ...( * D)
			diag_indices: typing.Final[npt.NDArray[np.int_]] = np.argwhere(f01 == 0)
...
def force_basis_potential(x: npt.NDArray[np.double], dim: int | None = None) -> npt.NDArray[np.double]:
	...
	"""
	mat: typing.Final[npt.NDArray[np.double]] = diabatic_to_force_basis(x, dim) # ...( * D) * N * N
	Vd: npt.NDArray[np.double] = diabatic_potential(x)
	if dim is None:
		Vd = Vd[..., np.newaxis, :, :] # ...( * 1) * N * N
	return np.swapaxes(mat, -2, -1) @ Vd @ mat
