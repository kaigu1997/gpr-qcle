...
FIGSIZE: tuple[float, float] = tuple(matplotlib.rcParams["figure.figsize"])[:2]
...
def read_input() -> tuple[npt.NDArray[np.double], npt.NDArray[np.double], npt.NDArray[np.double], npt.NDArray[np.double], npt.NDArray[np.double], npt.NDArray[np.double], int, int, float]:
	...
	def cutoff(x: float) -> float:
		...
		log_x: typing.Final[float] = np.log10(np.abs(x))
		n: typing.Final[int] = int(np.floor(log_x))
	...
	with open("input", "r", encoding="UTF-8") as in_f:
		...
		r0: npt.NDArray[np.double] = np.concatenate((x0, p0))
		...
		sigma_r0: npt.NDArray[np.double] = np.concatenate((pes.HBAR / 2.0 / sigma_p0, sigma_p0))
		dx: npt.NDArray[np.double] = np.array(lines[11][:-1].split(" "), dtype=np.double)
		assert pes.DIM % dx.size == 0
		dx = np.vectorize(cutoff)(np.minimum(np.tile(dx, pes.DIM // dx.size), math.pi * pes.HBAR / (r0[pes.DIM:] + 3.0 * sigma_r0[pes.DIM:]))) # 2 grids per de Broglie wavelength
		...
		return mass, r0, sigma_r0, dx, init_ppl, init_phase, int(round(output_interval / dt)), max(int(round(reopt_interval / output_interval)), 1), dt
...
def read_data(filename: str) -> npt.NDArray[np.double]:
	...
	ticks: typing.Final[int] = data.shape[0] // pes.NUM_ELM
	length: typing.Final[int] = int(np.round(np.power(data.shape[1], 1.0 / pes.PHASEDIM)))
...
def get_grids(r0: npt.NDArray[np.double], n_grids: int) -> list[npt.NDArray[np.double]]:
	...
	"""
	xmax: typing.Final[npt.NDArray[np.double]] = 2.0 * np.abs(r0[:pes.DIM])
	xmin: typing.Final[npt.NDArray[np.double]] = -xmax
	dx: typing.Final[npt.NDArray[np.double]] = (xmax - xmin) / (n_grids - 1)
	pmin: typing.Final[npt.NDArray[np.double]] = r0[pes.DIM:] - math.pi * pes.HBAR / 2.0 / dx
	pmax: typing.Final[npt.NDArray[np.double]] = r0[1] + math.pi * pes.HBAR / 2.0 / dx
...
def plot_average() -> None:
	...
	"""
	NUM_MOMENTS: typing.Final = pes.PHASEDIM * (pes.PHASEDIM + 3) // 2
	...
	averages_all: typing.Final[npt.NDArray[np.double]] = np.loadtxt(AVERAGE_FILENAME + DATA_EXTENSION)
...
def plot_error(output_interval: float) -> None:
	...
	assert isinstance(axs, np.ndarray)
	error: typing.Final[npt.NDArray[np.double]] = np.loadtxt(ERROR_FILENAME + DATA_EXTENSION).reshape(-1, 3, pes.NUM_ELM)
	ticks: typing.Final[npt.NDArray[np.double]] = np.arange(error.shape[0]) * output_interval
...
def plot_parameters(ticks: npt.NDArray[np.double]) -> None:
	...
	assert isinstance(axs, np.ndarray)
	parameters: typing.Final[npt.NDArray[np.double]] = np.loadtxt(PARAMETER_FILENAME + DATA_EXTENSION).reshape(-1, pes.NUM_ELM, pes.PHASEDIM)[:-1]
	assert ticks.size == parameters.shape[0]
	parameter_names: typing.Final[list[str]] = [utility.dimension_name(iDim) for iDim in range(pes.PHASEDIM)]
...
class PNLogNorm(matplotlib.colors.Normalize):
	...
	def __init__(self, abs_min: float = sys.float_info.min, abs_max: float = sys.float_info.max, clip: bool = False):
		...
		self.__abs_min: typing.Final[float] = abs_min
		self.__abs_max: typing.Final[float] = abs_max
		self.__log2_abs_min: typing.Final[float] = math.log2(abs_min)
		self.__log2_abs_max: typing.Final[float] = math.log2(abs_max)
		self.__log2_diff: typing.Final[float] = self.__log2_abs_max - self.__log2_abs_min
...
def get_centered(abs_max: float, cmap: str | matplotlib.colors.Colormap) -> tuple[matplotlib.colors.CenteredNorm, npt.NDArray[np.double], npt.NDArray[np.double]]:
	...
	centered_norm: typing.Final[matplotlib.colors.CenteredNorm] = matplotlib.colors.CenteredNorm(0.0, math.ceil(abs_max / abs_max_10_level) * abs_max_10_level, True)
	centered_levels: typing.Final[npt.NDArray[np.double]] = np.linspace(-centered_norm.halfrange, centered_norm.halfrange, cmap.N // 8 * 8 + 1, True)
...
def get_posneg_log(abs_min: float, abs_max: float, cmap: str | matplotlib.colors.Colormap) -> tuple[PNLogNorm, npt.NDArray[np.double], npt.NDArray[np.double]]:
	...
	if isinstance(cmap, str):
		cmap = matplotlib.colormaps[cmap]
	num_ticks: typing.Final[int] = cmap.N // 8 * 8 + 3
	pn_log_norm: typing.Final[matplotlib.colors.Normalize] = PNLogNorm(math.pow(10, lb_log10), math.pow(10, ub_log10), True)
...
class DensityMatrixDrawer:
	...
	"""
	__CMAP: typing.Final = "seismic"
	FILENAME_PREFIX: typing.Final = "dm"
	__PICNAME_NO_DIGITS: typing.Final = FILENAME_PREFIX + "_{{:0{}}}" + FIGURE_EXTENSION
	...
	def __get_divisors(
		...
		def get_single_divisor(dpi: float, size: int) -> int:
			...
			if dpi > size:
				...
				all_factors: typing.Final[set[int]] = set(functools.reduce(list.__add__, ([i, (size - 1) // i] for i in range(1, int(np.sqrt(size - 1)) + 1) if (size - 1) % i == 0)))
				factors_below_lim: typing.Final[set[int]] = {i for i in all_factors if i < size / dpi}
				return max(factors_below_lim)

		figsize_in_dpi: typing.Final[npt.NDArray[np.double]] = fig.get_size_inches() * fig.dpi
	...
	def __init__(
		...
		self.__output_interval: typing.Final[float] = output_interval
		self.dm_data: typing.Final[npt.NDArray[np.double] | None] = dm_data
		self.__grid_data: typing.Final[npt.NDArray[np.double] | None] = grid_data # grid_data.shape[0] may not be dm_data.shape[0]
		self.__draw_scattered: typing.Final[bool] = draw_scattered
		self.__draw_rescaled: typing.Final[bool] = draw_rescaled
		...
		self.picname: typing.Final[str] = __class__.__PICNAME_NO_DIGITS.format(num_digits_of_ticks)
		...
		if self.__draw_rescaled:
			...
			LOG_MAX = 1.5 * np.max(np.abs(init_dist.weight)) / (2.0 * math.pi) ** pes.DIM / init_dist.sigma_r0.prod()
			CENTER_MAX = LOG_MAX
		else:
		...
		self.__super_title: typing.Final[str] = self.__title[0] + "Partial Wigner-Transformed Density Matrix"
		...
		if self.__draw_rescaled:
			if scale is not None:
				...
				with np.errstate(divide="ignore"):
					rescale_factors = np.where(np.any(self.dm_data[frame_index].reshape(pes.NUM_ELM, -1) != 0, -1), 1.0 / np.max(np.abs(self.dm_data[frame_index].reshape(pes.NUM_ELM, -1)), -1), 0)
		else:
...
class WavefunctionPlotter:
	...
	__LABEL_TEMPLATE: typing.Final = "Surface {}"
	FILENAME_PREFIX: typing.Final = "wfn"
	__PICNAME_NO_DIGITS: typing.Final = FILENAME_PREFIX + "_{{:0{}}}.png"
	...
	def __init__(
		...
		# directly save from parameters
		self.__output_interval: typing.Final[float] = output_interval
		self.wfn_sqnm: typing.Final[npt.NDArray[np.double] | None] = marginal_data
		self.__draw_rescaled: typing.Final[bool] = draw_rescaled
		self.__title: typing.Final[str] = ("Rescaled " if self.__draw_rescaled else "") + "Marginal on each surfaces"
		# data, grids, and filename
		num_digits_of_ticks: int
		...
		labels: typing.Final[list[str]] = [utility.dimension_name(iDim) for iDim in range(pes.PHASEDIM)]
		titles: typing.Final[list[str]] = ["Position", "Momentum"]
		for iDim in range(pes.PHASEDIM):
			...
			if self.__draw_rescaled:
				...
				max_y = 1.5 * np.max(np.abs(init_dist.weight.diagonal())) / np.sqrt(2.0 * math.pi) / init_dist.sigma_r0[iDim]
			else:
...
def main() -> None:
	...
	output_steps: int
	dt: float
	_, r0, _, _, _, _, output_steps, _, dt = read_input()
	output_interval: typing.Final[float] = output_steps * dt
	...
	try:
		...
		dm_total_ticks: typing.Final[int] = dm_drawer.dm_data.shape[0]
	...
	try:
		...
		wfn_total_ticks: typing.Final[int] = wfn_plotter.wfn_sqnm.shape[0]
